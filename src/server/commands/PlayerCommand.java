...
public class PlayerCommand {
	...
	public static class Check extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			c.getPlayer().dropMessage(6, ""/* + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getCSPoints(2)) + " Cash, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getEPoints()) + " Event Points, " */ + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getDPoints()) + " DP, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getVPoints()) + " VP, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getIntNoRecord(GameConstants.BOSS_PQ)) + " Dojo points | " + (Math.round(c.getPlayer().getEXPMod()) * 100) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + "% EXP, " + (Math.round(c.getPlayer().getDropMod()) * 100) * Math.round(c.getPlayer().getStat().dropBuff / 100.0) + "% Drop, " + Math.round(c.getPlayer().getStat().mesoBuff / 100.0) * 100 + "% Meso");
			c.getPlayer().dropMessage(6, "" + FileoutputUtil.CurrentReadable_TimeGMT() + " GMT | Logged in " + "x" + " minutes | " + NumberFormat.getNumberInstance(Locale.US).format(c.getLatency()) + " Ping");
			c.getPlayer().dropMessage(6, "" + "z(x)" + " | " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().dam_r) + "% Damage,  " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().watk) + ", " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_atk) + "% ATT,  " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_str) + "% STR, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_dex) + "% DEX, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_int) + "% INT, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_luk) + "% LUK, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_hp) + "% HP, " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getStat().percent_mp) + "% MP");
			if (c.getPlayer().getLevel() > 249) {
				if (c.getPlayer().getNeededExp() > 0) {
					c.getPlayer().dropMessage(6, "EXP: " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getExp()) + " / " + NumberFormat.getNumberInstance(Locale.US).format(c.getPlayer().getNeededExp()) + " (" + (c.getPlayer().getExp() / c.getPlayer().getNeededExp()) + "%)");
				}
			}
			c.getPlayer().dropMessage(6, "Position: " + c.getPlayer().getPosition() + ", True Position: " + c.getPlayer().getTruePosition());
			c.removeClickedNPC();
			NPCScriptManager.getInstance().dispose(c);
			c.getSession().write(CWvsContext.enableActions());
			return 1;
