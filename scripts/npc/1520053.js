// Black Mage's Shadow (1520053)  Black Mage Instance NPC
var status = -1;
...
function start() {
    ...
    if (cm.getPlayer().getClient().getChannel() != 1 && cm.getPlayer().getClient().getChannel() != 8 && cm.getPlayer().getClient().getChannel() != 9) {
        cm.sendOk("Black Mage may only be attempted on channel 1,8,9.");
        cm.dispose();
        return;
    }
    var em = cm.getEventManager("BlackMBattle");

    if (em == null) {
        cm.sendOk("Black Mage is disabled.");
        cm.dispose();
        return;
    }
