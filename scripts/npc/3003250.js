// Lucid - (3003250)
// Lucid Battle Starter
var status = -1;

function start() {
	if (cm.getPlayer().getMapId() == 450004450) {
    ...
    var em = cm.getEventManager("LucidBattle");
    ...
    var squadAvailability = cm.getSquadAvailability("Lucid");
	...
	var type = cm.isSquadLeader("Lucid");
	if (type == -1) {
	    cm.sendOk("The squad has ended, please re-register.");
	    cm.dispose();
	} else if (type == 0) {
	    var memberType = cm.isSquadMember("Lucid");
	    if (memberType == 2) {
	...
	// TODO viewing!
			...
			var eim = cm.getDisconnected("LucidBattle");
			if (eim == null) {
				var squd = cm.getSquad("Lucid");
				if (squd != null) {
	    if (time + (24 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
		cm.sendOk("You have already went to Cygnus in the past 24 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (24 * 3600000)));
		cm.dispose();
...
function action(mode, type, selection) {
	...
	case 0:
	    if (mode == 1) {
			if (cm.registerSquad("Lucid", 5, " has been named the Leader of the squad. If you would you like to join please register for the Expedition Squad within the time period.")) {
	...
	case 5:
	    if (selection == 0) { // join
		var ba = cm.addMember("Lucid", true);
		if (ba == 2) {
		...
		var baa = cm.addMember("Lucid", false);
		if (baa == 1) {
	...
	case 10:
		...
		if (selection == 0) {
		    ...
		    status = 11;
		    if (!cm.getSquadList("Lucid", 1)) {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
			cm.dispose();
		    }
		} else if (selection == 2) {
		    status = 12;
		    if (!cm.getSquadList("Lucid", 2)) {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
			cm.dispose();
		    }
		} else if (selection == 3) { // get insode
		    if (cm.getSquad("Lucid") != null) {
			var dd = cm.getEventManager("LucidBattle");
			dd.startInstance(cm.getSquad("Lucid"), cm.getMap(), 160109);
		    } else {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
		    }
		    cm.dispose();
	    ...
	    break;
	case 11:
	    cm.banMember("Lucid", selection);
	    cm.dispose();
	    break;
	case 12:
	    if (selection != -1) {
		cm.acceptMember("Lucid", selection);
	    }
	    cm.dispose();
	    break;
    }
