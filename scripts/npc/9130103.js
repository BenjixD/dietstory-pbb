// Ayame (9130103)
// PrincessNo Battle Starter

var status = -1;

function start() {
	if (cm.getPlayer().getMapId() == 811000100 || cm.getPlayer().getMapId() == 811000500) {
    ...
    var em = cm.getEventManager("PNoBattle");
    ...
    var squadAvailability = cm.getSquadAvailability("PNo");
	...
	var type = cm.isSquadLeader("PNo");
	if (type == -1) {
	    cm.sendOk("The squad has ended, please re-register.");
	    cm.dispose();
	} else if (type == 0) {
	    var memberType = cm.isSquadMember("PNo");
	    if (memberType == 2) {
	...
	// TODO viewing!
			...
			var eim = cm.getDisconnected("PNoBattle");
			if (eim == null) {
				var squd = cm.getSquad("PNo");
				if (squd != null) {
	    if (time + (24 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
		cm.sendOk("You have already went to Cygnus in the past 24 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (24 * 3600000)));
		cm.dispose();
...
function action(mode, type, selection) {
	...
	case 0:
	    if (mode == 1) {
			if (cm.registerSquad("PNo", 5, " has been named the Leader of the squad. If you would you like to join please register for the Expedition Squad within the time period.")) {
	...
	case 5:
		...
		var baa = cm.addMember("PNo", false);
		if (baa == 1) {
	...
	case 10:
		...
		if (selection == 0) {
		    ...
		    status = 11;
		    if (!cm.getSquadList("PNo", 1)) {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
			cm.dispose();
		    }
		} else if (selection == 2) {
		    status = 12;
		    if (!cm.getSquadList("PNo", 2)) {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
			cm.dispose();
		    }
		} else if (selection == 3) { // get insode
		    if (cm.getSquad("PNo") != null) {
			var dd = cm.getEventManager("PNoBattle");
			dd.startInstance(cm.getSquad("PNo"), cm.getMap(), 160109);
		    } else {
			cm.sendOk("Due to an unknown error, the request for squad has been denied.");
		    }
		    cm.dispose();
	    ...
	    break;
	case 11:
	    cm.banMember("PNo", selection);
	    cm.dispose();
	    break;
	case 12:
	    if (selection != -1) {
		cm.acceptMember("PNo", selection);
	    }
	    cm.dispose();
	    break;
    }
